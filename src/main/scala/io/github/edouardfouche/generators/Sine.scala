...
case class Sine(nDim: Int, noise: Double, noisetype: String, discretize: Int)(period: Option[Double] = Some(1)) extends ParameterizedDataGenerator {
  ...
  protected def getPoints(n: Int): Array[Array[Double]] = {
    (1 to n).toArray.map { _ =>
      ...
      for (y <- 2 to nDim) {
        data = data ++ Array((math.sin(data.sum * 2 * math.Pi * s) + 1) / 2)
      }
      data
    }
  }
